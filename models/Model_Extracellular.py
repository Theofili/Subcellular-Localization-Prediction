...
def model_extracellular(df_extracellular):
    ...
    training_args = transformers.TrainingArguments(
        output_dir="./extracellular_results",
        eval_strategy="epoch",
        save_strategy="epoch",
        logging_steps=10,
    ...
    tokenizer_extracellular=transformers.AutoTokenizer.from_pretrained(
        model_name,
    ...
    tokenizer_extracellular.eos_token = '[EOS]'
    tokenizer_extracellular.pad_token = '[PAD]'
    #print(tokenizer, '/n')
    print('Done!')
    ...
    # Split the dataframe into training, validation, and test sets --- 80%/10%/10%
    train_df, temp_df = train_test_split(df_extracellular, test_size=0.2, random_state=42)
    ...
    train_dataset = SupervisedDataset(tokenizer=tokenizer_extracellular, data_path=temp_train_path, kmer=-1)
    val_dataset = SupervisedDataset(tokenizer=tokenizer_extracellular, data_path=temp_val_path, kmer=-1)
    test_dataset = SupervisedDataset(tokenizer=tokenizer_extracellular, data_path=temp_test_path, kmer=-1)

    data_collator = DataCollatorForSupervisedDataset(tokenizer=tokenizer_extracellular)
    ...
    model_01.config.pad_token_id = tokenizer_extracellular.pad_token_id
    #print(model, '/n')
    print('Done!')
    ...
    return model_01, tokenizer_extracellular, data_expe
