...
def model_mitochondria(df_mitochondria):
    ...
    training_args = transformers.TrainingArguments(
        output_dir="./mitochondria_results",
        eval_strategy="epoch",
        save_strategy="epoch",
        logging_steps=10,
    ...
    tokenizer_mitochondria=transformers.AutoTokenizer.from_pretrained(
        model_name,
    ...
    tokenizer_mitochondria.eos_token = '[EOS]'
    tokenizer_mitochondria.pad_token = '[PAD]'
    #print(tokenizer, '/n')
    print('Done!')
    ...
    train_df, temp_df = train_test_split(df_mitochondria, test_size=0.2, random_state=42)
    val_df, test_df = train_test_split(temp_df, test_size=0.5, random_state=42)

    # Make the paths -- NEED TO CHANGE THE FOLDER --
    temp_train_path = "data/mitochondria_temp_train.csv"
    temp_val_path = "data/mitochondria_temp_val.csv"
    temp_test_path = "data/mitochondria_temp_test.csv"
    ...
    train_dataset = SupervisedDataset(tokenizer=tokenizer_mitochondria, data_path=temp_train_path, kmer=-1)
    val_dataset = SupervisedDataset(tokenizer=tokenizer_mitochondria, data_path=temp_val_path, kmer=-1)
    test_dataset = SupervisedDataset(tokenizer=tokenizer_mitochondria, data_path=temp_test_path, kmer=-1)

    data_collator = DataCollatorForSupervisedDataset(tokenizer=tokenizer_mitochondria)
    ...
    model_01.config.pad_token_id = tokenizer_mitochondria.pad_token_id
    #print(model, '/n')
    print('Done!')
    ...
    model_dir = "C:/Users/theos/SubCel/SubCel/models/model_mitochondria"
    model_01.save_pretrained(model_dir)
    tokenizer_mitochondria.save_pretrained(model_dir)

    ## Need to check if the metrics are good
    return model_01, tokenizer_mitochondria, data_expe


import pandas as pd
df = pd.read_csv('C:/Users/theos/SubCel/SubCel/data/model_data/model_mitochondria_data.csv')
model, tokenizer, metrics = model_mitochondria(df)
