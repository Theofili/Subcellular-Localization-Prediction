...
def model_reticulum(df_reticulum):
    ...
    tokenizer_reticulum=transformers.AutoTokenizer.from_pretrained(
        model_name,
    ...
    tokenizer_reticulum.eos_token = '[EOS]'
    tokenizer_reticulum.pad_token = '[PAD]'
    #print(tokenizer, '/n')
    print('Done!')
    ...
    train_df, temp_df = train_test_split(df_reticulum, test_size=0.2, random_state=42)
    val_df, test_df = train_test_split(temp_df, test_size=0.5, random_state=42)

    # Make the paths -- NEED TO CHANGE THE FOLDER --
    temp_train_path = "data/reticulum_temp_train.csv"
    temp_val_path = "data/reticulum_temp_val.csv"
    temp_test_path = "data/reticulum_temp_test.csv"
    ...
    train_dataset = SupervisedDataset(tokenizer=tokenizer_reticulum, data_path=temp_train_path, kmer=-1)
    val_dataset = SupervisedDataset(tokenizer=tokenizer_reticulum, data_path=temp_val_path, kmer=-1)
    test_dataset = SupervisedDataset(tokenizer=tokenizer_reticulum, data_path=temp_test_path, kmer=-1)

    data_collator = DataCollatorForSupervisedDataset(tokenizer=tokenizer_reticulum)
    ...
    model_01.config.pad_token_id = tokenizer_reticulum.pad_token_id
    #print(model, '/n')
    print('Done!')
    ...
    model_dir = "C:/Users/theos/SubCel/SubCel/models/model_reticulum"
    model_01.save_pretrained(model_dir)
    tokenizer_reticulum.save_pretrained(model_dir)

    ## Need to check if the metrics are good
    return model_01, tokenizer_reticulum, data_expe


import pandas as pd
df = pd.read_csv('C:/Users/theos/SubCel/SubCel/data/model_data/model_reticulum_data.csv')
